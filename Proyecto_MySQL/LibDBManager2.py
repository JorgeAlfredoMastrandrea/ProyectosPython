...
def BorrarTablaLocal(db , cursor , tabla):
    accionExitosa = False
    try:
        print(f"{Fore.BLUE}{Style.BRIGHT}...borrando datos de la tabla : {tabla} en {db.database} ...{Style.RESET_ALL}")
        ...
        print(f"{Fore.RED}{Style.BRIGHT}...ERROR en (LibDBManager2.py --> BorrarTablaLocal(db , cursor , tabla)) no se pudo borrar los datos de la tabla...{Style.RESET_ALL}")
    else:
        print(f"{Fore.GREEN}{Style.BRIGHT}...datos borrados de la tabla {tabla} de la base {db.database} ...{Style.RESET_ALL}")
        accionExitosa = True
    return accionExitosa
